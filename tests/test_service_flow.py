...
class ServiceFlowTests(unittest.TestCase):
    ...
    def test_process_telegram_update_keeps_previous_visible_answer_while_final_delivery_pending(self) -> None:
        auth = AuthState(
            bot_token="token",
            telegram_user_id=11,
            telegram_chat_id=22,
        ...
        session.last_delivered_output_text = "7"
        session.pending_output_text = "7 `LC_TIME`"
        session.streaming_output_text = "7 `LC_TIME`"
        ...
        updated = store.get_or_create_telegram_session(auth)
        self.assertEqual(updated.last_delivered_output_text, "7")
        self.assertEqual(updated.streaming_message_ids, [])
        self.assertIsNone(updated.streaming_message_id)
        self.assertEqual(telegram.deletes, [])
    ...
    def test_reconcile_pending_final_deliveries_retries_without_topic_after_topic_closed_failure(self) -> None:
        auth = AuthState(
            bot_token="token",
            telegram_user_id=11,
            telegram_chat_id=22,
    ...
    def test_reconcile_pending_final_deliveries_scans_all_telegram_sessions(self) -> None:
        ...
        store = SessionStore(self.paths)
        session = store.get_or_create_telegram_session(
            AuthState(
                bot_token="token",
                telegram_user_id=11,
                telegram_chat_id=44,
